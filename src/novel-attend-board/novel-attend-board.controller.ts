...
export class NovelAttendBoardController {
  ...
  @FindDetailBoard()
  @Get(':roomId')
  async findByIdDetail(@Param('roomId', ParseIntPipe) novelRoomId: number, @CurrentUser() user: UserEntity) {
    return await this.novelAttendBoardService.findById(novelRoomId, user);
  }
